...
/************************************************************************************/
//realtime stuff
//copy paste of above for now

static inline int32_t steal_work_rt(pthread_arg_rt_t* all_args, int32_t n_threads) {
	int32_t i, c_i = -1;
	int32_t k;
	for (i = 0; i < n_threads; ++i){
        pthread_arg_rt_t args = all_args[i];
        //fprintf(stderr,"endi : %d, starti : %d\n",args.endi,args.starti);
		if (args.endi-args.starti > STEAL_THRESH) {
            //fprintf(stderr,"gap : %d\n",args.endi-args.starti);
            c_i = i;
...
// for realtime stuff
void* pthread_rt_single(void* voidargs) {
    int32_t i;
    pthread_arg_rt_t* args = (pthread_arg_rt_t*)voidargs;
    sigfish_read_t* db = args->db;
    sigfish_state_t* core = args->state;
...
#else
    pthread_arg_rt_t* all_args = (pthread_arg_rt_t*)(args->all_pthread_args);
    //adapted from kthread.c in minimap2
    for (;;) {
		i = __sync_fetch_and_add(&args->starti, 1);
		if (i >= args->endi) {
	...
	while ((i = steal_work_rt(all_args,core->num_thread)) >= 0){
		args->func(core,db,i);
    }
#endif
...
void pthread_rt(sigfish_state_t* state, sigfish_read_t* db, void (*func)(sigfish_state_t*,sigfish_read_t*,int)){
    //create threads
    pthread_t tids[state->num_thread];
    pthread_arg_rt_t pt_args[state->num_thread];
    int32_t t, ret;
    int32_t i = 0;
    int32_t num_thread = state->num_thread;
    int32_t step = (state->n_rec + num_thread - 1) / num_thread;
    //todo : check for higher num of threads than the data
    //current works but many threads are created despite

    //set the data structures
    for (t = 0; t < num_thread; t++) {
        pt_args[t].state = state;
        pt_args[t].db = db;
        pt_args[t].starti = i;
        i += step;
        if (i > state->n_rec) {
            pt_args[t].endi = state->n_rec;
        } else {
            pt_args[t].endi = i;
        }
        pt_args[t].func=func;
    ...
    //create threads
    for(t = 0; t < state->num_thread; t++){
        ret = pthread_create(&tids[t], NULL, pthread_rt_single,
                                (void*)(&pt_args[t]));
        NEG_CHK(ret);
    }
...
void work_rt(sigfish_state_t* state, sigfish_read_t* db, void (*func)(sigfish_state_t*,sigfish_read_t*,int)){
    ...
    if (state->num_thread == 1) {
        int32_t i=0;
        for (i = 0; i < state->n_rec; i++) {
            func(state,db,i);
        }
    }

    else {
        pthread_rt(state,db,func);
    }
}