...
[ -z ${SIGFISH} ] && export SIGFISH=sigfish
${SIGFISH} --version &> /dev/null || { echo -e $RED"sigfish not found! Either put sigfish under path or set SIGFISH variable, e.g.,export SIGFISH=/path/to/sigfish"$NORMAL; exit 1;}
...
rm -f sigfish_${read_id}.tmp sigfish_${read_id}.prefix.tmp

${SLOW5TOOLS} get --to slow5 ${FILE} ${read_id} | grep -v '^[#@]' | awk '{print $8}' > sigfish_${read_id}.tmp || { echo -e $RED"Error: failed to get read_id ${read_id} from ${FILE}"$NORMAL; exit 1;}
${SIGFISH} real ${FILE} | grep ${read_id}  | cut -f 3,4,5,6 | sed 's/\./-1/g'  > sigfish_${read_id}.prefix.tmp || { echo -e $RED"Error: failed to get segments for read_id ${read_id} from ${FILE}"$NORMAL; exit 1;}

if [[ "${SIGTK_PLOT_MTD}" == "matlab" ]]; then

    matlab.exe -nosplash -nodesktop -minimize -r "
    a=dlmread('sigfish_${read_id}.tmp'); x=dlmread('sigfish_${read_id}.prefix.tmp');
    x1=x(:,[1:2]);
    x2=x(:,[3:4]);
    y=[1200,1200];
    plot(a); hold on;
...
signal_fpath = 'sigfish_' + read_id + '.tmp'
prefix_fpath = 'sigfish_' + read_id + '.prefix.tmp'
